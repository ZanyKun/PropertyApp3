...
function myFunction(){   
    var current = document.getElementById('autoSearch').value;
    if(current != undefined && current.length >= 2){
        var url = 'http://localhost:9230/properties/_search?q=projectName:' + current;
    console.log(url) + "&city:" + current;
    fetch('http://localhost:9230/properties/_search?q=projectName:' + current).then(function(response){
        return response.json();}).then((data) => {
        var firstArr = Object.values(data["hits"])[2];
        console.log(firstArr);
        var secondArr = []
        
        var i;
        for(i = 0; i < firstArr.length; i++){
            secondArr.push(firstArr[i]._source.projectName); 
        }
